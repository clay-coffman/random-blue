...
Browser /founder
  → (optional) POST /api/v1/founder-passports/enrich  (Agent 2)
      → fetch the supplied URL, strip HTML to text, ask Anthropic
        for a structured FounderPassportInput shape (county, city,
        stage, industry, business_type, identity tags, needs)
      → ~5–10s; never throws — returns { degraded: true, fields: [] }
        on fetch error, timeout, parse failure, or denylist match
