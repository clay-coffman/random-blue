...
A production application for Utah's Governor's Office of Economic
...
persona descriptions, required profile fields, and customer priorities
(30% usability / 25% tech / 25% design / 20% innovation).
...
- **ID prefixes:**
  ...
  - `sc_*` — saved companies *(Phase 6 / post-launch — Agent 8 adds)*
  - `irq_*` — intro requests *(Phase 6 / post-launch)*
...
user-flow coverage: `../implementation-plan.md`.
...
- **Don't gold-plate.** No exhaustive tests, no comprehensive error
  handling, no premature optimization. Ship real failure UX — broken
  share URLs, 5xxs on real input, mobile regressions, dead empty/error
  states — matter more than cosmetic polish, because real founders
  will hit these paths before they hit the happy path.
- **User-facing first.** User-facing surfaces are the priority
  *because* they are what real founders / investors / business owners
  hit on the live site. The persona test fixtures make the happy path
  easy to regression-check; real users hit the failure paths first.
  Don't ship something user-pretty that breaks in production.
