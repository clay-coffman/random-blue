...
A comprehensive catalog of transactional emails organized by category, plus
recommended email combinations for different app types.
...
Apps where user accounts and security are core (login systems, identity
providers, account management).
...
**Note:** Healthcare apps have strict requirements. Emails should contain
minimal PHI and link to secure portals for sensitive information.
...
**When to send:** When security-relevant events occur (login from new device,
password change, etc.).
...
**When to send:** Immediately after successful account creation and
verification.

**Purpose:** Welcome new users and guide them to next steps (must not be
promotional).
...
**When to send:** When user changes account settings (email, password, profile,
etc.).
...
**Note:** General feature announcements are marketing emails. Only send as
transactional if the change directly affects an active feature the user is
using.
