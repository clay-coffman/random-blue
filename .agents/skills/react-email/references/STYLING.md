...
Use the `Tailwind` component for styling if the project uses Tailwind CSS.
Otherwise, use inline styles.
...
Email clients don't support `rem` units. Always use `pixelBasedPreset` in your
Tailwind configuration to convert rem-based utilities to pixels:
...
Use consistent spacing that respects content hierarchy. Larger margins for
headings, smaller for body text.
...
- **Secondary**: Supporting color for borders, backgrounds, less prominent
  elements
...
Create a centralized Tailwind config file that all email templates import. Using
`satisfies TailwindConfig` provides intellisense support for all configuration
options:
...
- **Always use the brand config** - Never hardcode colors in individual
  templates
...
- **Ensure contrast** - Test that text is readable against backgrounds (WCAG AA:
  4.5:1 ratio)
...
- **Logo and images**: Host on a CDN or public URL. For local development, place
  in `emails/static/`.
- **Custom fonts**: Use the `Font` component with a web font URL (Google Fonts,
  Adobe Fonts, or self-hosted).

**Example prompt for gathering brand info:**

> "Before I create your email template, I need some brand information to ensure
> consistency. Could you provide:
>
